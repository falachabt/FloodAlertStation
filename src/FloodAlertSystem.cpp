...
void FloodAlertSystem::update()
{
    ...
    // If we have local sensor data and LED indicator, update it
    if (_ledIndicator != nullptr && _sensors.size() > 0) {
        // Find water level sensors
        for (auto sensor : _sensors) {
            if (strcmp(sensor->getName(), "WaterLevel") == 0) {
                ...
                if (count >= 3) {
                    // updateLEDIndicator(data[0], (uint8_t)data[2]);
                }
                break;
            }
        }
...
void FloodAlertSystem::handleSensorData(const float* data, uint8_t count, const uint8_t* mac, const char* sensorName) {
    // Existing code from your handleSensorData method...
    ...
    // Update LED indicator based on water level data
    if (_ledIndicator != nullptr) {
        updateLEDIndicator(_remoteSensors[idx].waterLevel, _remoteSensors[idx].category);
    }
...
void FloodAlertSystem::updateLEDIndicator(float waterLevel, uint8_t category) {
    Serial.println("\n--- Updating LED Indicator ---");
    Serial.print("Water Level: ");
